...
def register_routes(app, get_db):
    ...
    # Função auxiliar para evitar Erro 500 se o banco de rede falhar
    def safe_db():
        db = get_db()
        if db is None:
            flash("Erro de Conexão: O banco de dados na rede (W:) está inacessível no momento.")
            return None
        return db
    ...
    def index():
        db = safe_db()
        if not db: return render_template('login.html') # Redireciona se falhar
    ...
    # 2. CONFIGURAÇÃO (Criação de Tabelas)
    ...
    def config():
        ...
        if request.method == 'POST':
            try:
                ...
                default_cols = ["contrato TEXT", "ano TEXT", "nome_da_obra TEXT", "data_base TEXT", "referencia TEXT"]
        ...
        if request.method == 'POST':
            ...
            if db:
                ...
                if res and check_password_hash(res['password'], pwd):
                    ...
                    return redirect(url_for('index'))
            flash("Credenciais inválidas ou erro de rede.")
